...
var jsc = {
	...
	appendDefaultCss : function (css) {
		jsc.appendCss(
			[
				...
				'.jscolor-wrap .jscolor-btn { position:absolute; overflow:hidden; white-space:nowrap; font:39px sans-serif; text-align:center; cursor:pointer; }',
			].join('\n')
		);
	},
	...
	redrawPosition : function () {
		...
		if (thisObj.container !== window.document.body) {
			...
			jsc._drawPosition(thisObj, x/*/1.2*/, y, positionValue, contractShadow);
...
*/
	...
	onPaletteSampleClick : function (e) {
		...
		thisObj.trigger('input');
		thisObj.trigger('change');
		//console.log(thisObj.valueElement);
	...
	pub : function (targetElement, opts) {
		...
		this.previewPadding = 24; // (px) space between color preview and content of the previewElement
		...
		this.paletteHeight = 48; // maximum height (px) of a row in the palette
		this.paletteSpacing = 50; // distance (px) between color samples in the palette
		this.hideOnPaletteClick = false; // when set to true, clicking the palette will also hide the color picker
		this.sliderSize = 48; // px
		this.crossSize = 24; // px
		this.closeButton = false; // whether to display the Close button
		this.closeText = 'Close';
		this.buttonColor = 'rgba(0,0,0,1)'; // CSS color
		this.buttonHeight = 54; // px
		this.padding = 12; // px
		...
		//I MODIFIED THIS FUNCTION TO REMOVE THE HASH IN THE INPUT VALUES
		...
		function drawPicker () {
			...
			p.wrap.style.width = pickerDims.borderW * 1.3 + 'px';
			p.wrap.style.height = pickerDims.borderH + 'px';
			p.wrap.style.zIndex = THIS.zIndex;

			// picker
			p.box.className = 'jscolor-picker';
			p.box.style.width = pickerDims.paddedW * 1.3 + 'px';
			p.box.style.height = pickerDims.paddedH * 1.2 + 'px';
			...
			for (var r = 0; r < pickerDims.palette.rows; r++) {
				for (var c = 0; c < pickerDims.palette.cols && si < THIS._palette.length; c++, si++) {
					...
					sc.style.width = (pickerDims.palette.cellW*3 - 2 * THIS.controlBorderWidth*3) + 'px';
					sc.style.height = (pickerDims.palette.cellH*3 - 2 * THIS.controlBorderWidth*3) + 'px';
			...
			p.btn.style.left = THIS.padding - 3 + 'px';
			p.btn.style.bottom = THIS.padding + 'px';
			p.btn.style.padding = '0 ' + 5 + 'px';
			p.btn.style.backgroundColor = '#171717';
			p.btn.style.borderRadius = '5px';
			...
			p.btn.onmousedown = function () {
				THIS.hide();
				deletePlayer(THIS.valueElement);
			};
